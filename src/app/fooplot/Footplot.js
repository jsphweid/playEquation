...
// interface "constants"
        ...
        this.FOOPLOT_TYPE_FUNCTION=0;
        this.FOOPLOT_TYPE_POLAR=1;
        this.FOOPLOT_TYPE_PARAMETRIC=2;
        this.FOOPLOT_TYPE_POINTS=3;
        this.FOOPLOT_MODE_MOVE=1;
        this.FOOPLOT_MODE_ZOOMBOX=2;
        // this.FOOPLOT_MODE_INTERSECTION=3;
        this.FOOPLOT_MODE_TRACE=4;
        this.FOOPLOT_UNITS_PI=1;
        this.FOOPLOT_UNITS_E=2;
        this.FOOPLOT_INSTANCES=[];
        this.FOOPLOT_MATH = this.getFOOPLOT_MATH();
        this.FOOPLOT_MSIE=(navigator.userAgent.toLowerCase().indexOf('msie')!=-1);
        this.FOOPLOT_TRANSITIONS=function() {
            let b = document.body||document.documentElement;
            let s = b.style;
            p = 'transition';
            if(typeof s[p] === 'string') {return true; }
            let v = ['Moz', 'Webkit', 'Khtml', 'O', 'ms'],
    ...
    fooplot() {
        ...
        this.FOOPLOT_INSTANCES.push(this);
        ...
        if(this.FOOPLOT_MSIE) this.container.unselectable=true;
        ...
        this.container.style.webkitUserSelect='none';
        this.container.style.MozUserSelect='none';
        if(this.FOOPLOT_MSIE) this.cover.unselectable=true;
        ...
        if(this.FOOPLOT_MSIE) this.subcontainer.unselectable=true;
        this.container.appendChild(this.subcontainer);

        this.recorder=new this.FooplotSVGRecorder();
        ...
        this.addToolButton=function(image,action,modeId,tooltip) {
            ...
            if(!modeId) {
                ...
                newtool.onclick=function() {
                    for (var i in this.FOOPLOT_INSTANCES) if(this.parentNode===this.FOOPLOT_INSTANCES[i].toolcontainer) var _self=this.FOOPLOT_INSTANCES[i];
                    this.selectMode(this);
                };
        ...
        this.toolsMode=[];
        this.selectedMode=this.FOOPLOT_MODE_MOVE;
        ...
        this.zoom=function(factor) {
            ...
            if(this.FOOPLOT_TRANSITIONS && this.canvas && this.canvas.style && factor!=1) {
            ...
            this.zoomTimeout=window.setTimeout(function(_self) {
                if(!_self) for (var i in this.FOOPLOT_INSTANCES) if(this.FOOPLOT_INSTANCES[i].zoomSelf) var _self=this.FOOPLOT_INSTANCES[i].zoomSelf;
                var centerx=(this.xmax+this.xmin)/2;
                var centery=(this.ymax+this.ymin)/2;
                this.xmax=(this.xmax-centerx)/this.zoomPendingFactor+centerx;
                this.xmin=(this.xmin-centerx)/this.zoomPendingFactor+centerx;
                this.ymax=(this.ymax-centery)/this.zoomPendingFactor+centery; // commented out because I don't want y to change
                this.ymin=(this.ymin-centery)/this.zoomPendingFactor+centery; // commented out because I don't want y to change
                this.zoomPendingFactor=1;
                this.reDraw();
                this.canvas.style.OTransition='color 0 ease'; // opera hack
                this.canvas.style.webkitTransition='';
                this.canvas.style.MozTransition='';
                this.canvas.style.msTransition='';
                this.canvas.style.webkitTransform='';
                this.canvas.style.MozTransform='';
                this.canvas.style.msTransform+='';
                this.canvas.style.OTransform='';
                this.onWindowChange([this.xmin,this.xmax,this.ymin,this.ymax]);
                this.zoomTimeout=null;
                this.zoomSelf=null;
            },this.FOOPLOT_TRANSITIONS?1:0,this); // changed this from 450 to 1
        };
        ...
        this.toolMove=this.addToolButton(
            ...
            null,this.FOOPLOT_MODE_MOVE,'Move');
        ...
        this.toolZoomBox=this.addToolButton(
            ...
            null,this.FOOPLOT_MODE_ZOOMBOX,'Zoom Box');
        ...
        //   null,this.FOOPLOT_MODE_TRACE,'Trace');
        ...
        //   null,this.FOOPLOT_MODE_INTERSECTION,'Find Intersections and Roots');
        ...
        this.selectMode(this.FOOPLOT_MODE_MOVE);
        ...
        this.drawLabels=function() {
            ...
            if((this.ymax-this.ymin)/realygrid<this.height/6) {
                for(y=Math.floor(this.ymin/realygrid)*realygrid;y<=this.ymax;y+=realygrid) {
                    ...
                    if(this.ygridunits===this.FOOPLOT_UNITS_PI) {
                        printy=this.tryToMakeFraction(y/this.vars.pi)+'π';
                    } else if(this.ygridunits===this.FOOPLOT_UNITS_E) {
            ...
            if((this.xmax-this.xmin)/realxgrid<this.width/6) {
                for(x=Math.floor(this.xmin/realxgrid)*realxgrid;x<=this.xmax;x+=realxgrid) {
                    px=(x-this.xmin)/(this.xmax-this.xmin)*this.width;
                    if(this.xgridunits===this.FOOPLOT_UNITS_PI) {
                        printx=this.tryToMakeFraction(x/this.vars.pi)+'π';
                    } else if(this.xgridunits===this.FOOPLOT_UNITS_E) {
        ...
        this.container.onmousemove=function(e) {
            ...
            for (var i in this.FOOPLOT_INSTANCES) if(this===this.FOOPLOT_INSTANCES[i].container) var _self=this.FOOPLOT_INSTANCES[i];
            if(e && e.preventDefault) e.preventDefault();
            if(e && (e.srcElement?e.srcElement:e.target).className==='fooplot-tool') return null;
            if(this.zoomTimeout) return null;
            var offx=this.container.offsetLeft;
            var offy=this.container.offsetTop;
            if(this.container.parentNode) {
                offx+=this.container.parentNode.offsetLeft;
                offy+=this.container.parentNode.offsetTop;
            }
            var mx=(!this.FOOPLOT_MSIE?e.pageX:(document.body.scrollLeft+event.clientX))-offx;
            var my=(!this.FOOPLOT_MSIE?e.pageY:(document.body.scrollTop+event.clientY))-offy;
            if(this.isMouseDown) {
                this.dpx=mx-this.dragInitX;
                this.dpy=my-this.dragInitY;
                switch(this.selectedMode) {
                    case this.FOOPLOT_MODE_MOVE:
                        this.subcontainer.style.left=this.dpx+'px';
                        this.subcontainer.style.top=this.dpy+'px';
                        break;
                    case this.FOOPLOT_MODE_ZOOMBOX:
                        this.zoomboxBox.style.width=(this.dpx)+'px';
                        this.zoomboxBox.style.height=(this.dpy)+'px';
                        break;
                    case this.FOOPLOT_MODE_TRACE:
                        var initx=mx/this.width*(this.xmax-this.xmin)+this.xmin;
                        var order=Math.pow(10,2-Math.floor(Math.log(this.xmax-this.xmin)/Math.log(10)));
                        initx=parseFloat(Math.floor(initx*order)/order);
                        var inity=(1-my/this.height)*(this.ymax-this.ymin)+this.ymin;
                        this.showTrace(initx,inity);
                        break;
                }
            }
        };
        ...
        this.container.onmousedown=function(e) {
            ...
            switch(this.selectedMode) {
                case this.FOOPLOT_MODE_ZOOMBOX:
                    this.zoomboxBox.style.left=mx+'px';
                    this.zoomboxBox.style.top=my+'px';
                    this.zoomboxBox.style.width='0px';
                    this.zoomboxBox.style.height='0px';
                    this.zoomboxBox.style.visibility='visible';
                    break;
            }
        }
        ...
        this.container.onmouseup=function(e) {
            ...
            if(this.isMouseDown) {
                ...
                switch(this.selectedMode) {
                    case this.FOOPLOT_MODE_MOVE:
                        if(this.zoomTimeout) return null;
                        var dx=this.dpx/this.width*(this.xmax-this.xmin);
                        var dy=this.dpy/this.height*(this.ymax-this.ymin);
                        this.xmin-=dx;
                        this.xmax-=dx;
                        this.ymin+=dy;
                        this.ymax+=dy;
                        ...
                        this.subcontainer.style.top='0px';
                        this.reDraw();
                        this.onWindowChange([this.xmin,this.xmax,this.ymin,this.ymax]);
                        break;
                    case this.FOOPLOT_MODE_ZOOMBOX:
                        this.zoomboxBox.style.visibility='hidden';
                        this.selectMode(this.FOOPLOT_MODE_MOVE);
                        if(parseInt(this.zoomboxBox.style.width)>5 && parseInt(this.zoomboxBox.style.height)>5) {
                            var newxmin=(parseInt(this.zoomboxBox.style.left)/this.width)*(this.xmax-this.xmin)+this.xmin;
                            var newymax=this.ymax-(parseInt(this.zoomboxBox.style.top)/this.height)*(this.ymax-this.ymin);
                            var newxmax=((parseInt(this.zoomboxBox.style.left)+parseInt(this.zoomboxBox.style.width))/this.width)*(this.xmax-this.xmin)+this.xmin;
                            var newymin=this.ymax-((parseInt(this.zoomboxBox.style.top)+parseInt(this.zoomboxBox.style.height))/this.height)*(this.ymax-this.ymin);
                            this.xmin=newxmin;
                            this.xmax=newxmax;
                            this.ymin=newymin;
                            this.ymax=newymax;
                        ...
                        break;
                    // case this.FOOPLOT_MODE_INTERSECTION:
                    // this.showIntersection(this.dragInitX/this.width*(this.xmax-this.xmin)+this.xmin);
                    // break;
                    // case this.FOOPLOT_MODE_TRACE:
                    // var initx=mx/this.width*(this.xmax-this.xmin)+this.xmin;
                    // var order=Math.pow(10,2-Math.floor(Math.log(this.xmax-this.xmin)/Math.log(10)));
        ...
        //     delta = -e.detail/3;
        //   }
        //   if(delta>0 && this.zoomPendingFactor<=8) this.zoom(1.25);
        //   else if(delta<0 && this.zoomPendingFactor>=0.125) this.zoom(0.8);
        //   else if(delta) this.zoom(1);
        // }
        ...
        this.drawFunction=function(jeq) {
            ...
            this.context.beginPath();
            var started=false;
            var pxstep=this.FOOPLOT_MSIE?1:0.25;
        ...
        this.parseEquation=function(eq,vars) {
            ...
            for(i=0;i<tokens.length;i++) {
                ...
                else if(tokens[i].length>0 && tokens[i].match(/^[a-z][a-z0-9]*$/i) && this.FOOPLOT_MATH[tokens[i]]===undefined) { this.parseEquationError="invalid variable or function: "+tokens[i];return null; }
            }
            ...
            // send to make music!
            // makeTone.mostRecentSring = (this.parseEquationJoinArray(tokens));
            ...
            for(var i in this.plots) {
                ...
                switch(this.plots[i].type) {
                    ...
                    case this.FOOPLOT_TYPE_POLAR:
                        ...
                        break;
                    case this.FOOPLOT_TYPE_PARAMETRIC:
                        this.context.strokeStyle=this.plots[i].options.color;
        ...
        this.addPlot=function(eq,type,options) {
            if(!type)    type=this.FOOPLOT_TYPE_FUNCTION;
            if(!options) options={};
        ...
        this.setGrid=function(g) {
            ...
            if(g[0]) {
                ...
                if((720*g[0]/this.vars.pi).toFixed(6).match(/0000$/)!==null) {
                    this.xgridunits=this.FOOPLOT_UNITS_PI;
                } else if((720*g[0]/this.vars.e).toFixed(6).match(/0000$/)!==null) {
                    this.xgridunits=this.FOOPLOT_UNITS_E;
                } else {
                    this.xgridunits=null;
                }
            } else {
            ...
            if(g[1]) {
                ...
                if((720*g[1]/this.vars.pi).toFixed(6).match(/0000$/)!==null) {
                    this.ygridunits=this.FOOPLOT_UNITS_PI;
                } else if((720*g[1]/this.vars.e).toFixed(6).match(/0000$/)!==null) {
                    this.ygridunits=this.FOOPLOT_UNITS_E;
                } else {
                    this.ygridunits=null;
                }
            } else {
